            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                ...
                if type(clicked_obj) == Button:
                    ...
                    if clicked_obj.tag == "Menu":
                        self.scenes[1].drawables[3].change_text("0.0 ms")
                    ...
                    if clicked_obj.tag == "Start":
                        ...
                        self.data_visualizer.time_taken = testsorts.test(self.data_visualizer.sort_name, self.data_visualizer.unsorted_data)
                        self.data_visualizer.display_time = True

                        if self.data_visualizer.display_time:
                            self.scenes[1].drawables[3].change_text(str(round(self.data_visualizer.time_taken, 1)) + " ms")
                        else:
                            self.scenes[1].drawables[3].change_text("0.0 ms")
...
def populate():
    sort_layout = [["Radix Sort", "Bitonic Sort", "Pancake Sort"],
                   ["Cocktail Shaker Sort", "Stooge Sort", "Cycle Sort"]]

    btns = []
    for i in range(len(sort_layout)):
        for j in range(len(sort_layout[i])):
            btns.append(Button(sort_layout[i][j], (SCREEN_RES[0] * (j + 1) / (len(sort_layout[i]) + 1),
                                                   SCREEN_RES[1] / 2 + SCREEN_RES[1] / 2 * (i + 1) / (
                                                               len(sort_layout) + 1)),
                               btn_size=(SCREEN_RES[0] / 5, SCREEN_RES[1] / 10)))
    ...
    click.append(
        Slider("Speed", (SCREEN_RES[0] * 5 / 8, YPAD * 2), SCREEN_RES[0] / 4, 1, 1000, suffix=" ms (between steps)",
               reversed=True))
    period = click[-1].value
    click.append(
        Slider("Data Size", (SCREEN_RES[0] * 5 / 8, YPAD * 3 + THUMB_SIZE[1]), SCREEN_RES[0] / 4, 2, 131072, pow2=True))
    data_size = click[-1].value

    draw = []
    data = DataVisualizer(data_size,
                          pygame.Rect(YPAD * 2, YPAD * 6, SCREEN_RES[0] - YPAD * 4, SCREEN_RES[1] - (YPAD * 6)),
                          SoundManager())
    ...
    click = []
    click.append(Slider("Sound Type", (SCREEN_RES[0] / 16, SCREEN_RES[1] / 8), SCREEN_RES[0] / 6, 0, 1,
                        tagset=["Triangle", "Sine"]))
    ...
    info_btns = []
    info_btns.append(
        Button("", (SCREEN_RES[0] / 2, SCREEN_RES[1] / 2), btn_size=(SCREEN_RES[0] / 1.2, SCREEN_RES[1] / 1.5)))
